...
- [x] 步骤37：优化标准化训练组件布局让内容可滚动
  - [x] 分析问题：课程头部卡片占据太多空间导致下方内容看不到
  - [x] 将整个组件改为ScrollArea可滚动布局
  - [x] 缩小课程头部卡片尺寸（p-6改为p-4，text-2xl改为text-xl）
  - [x] 缩小学习进度条高度（h-2改为h-1.5）
  - [x] 缩小课程统计图标和文字（h-4改为h-3，text-lg改为text-base）
  - [x] 缩小Tab标签尺寸（text-xs，图标h-3 w-3）
  - [x] 缩小课程列表项尺寸（p-4改为p-3，text-sm改为text-xs）
  - [x] 缩小课程内容图片高度（h-64改为h-48）
  - [x] 缩小测试题目和按钮尺寸（text-sm改为text-xs，size="lg"改为size="sm"）
  - [x] 确保所有内容可以通过滚动完整查看
  - [x] 代码质量检查通过（105个文件）
- [x] 步骤38：添加AI数字人功能
  - [x] 创建AIDigitalHuman组件（数字人形象+口型动画+国家切换器）
  - [x] 支持5个国家切换：越南（使用用户图片）、新加坡、马来西亚、印尼、菲律宾
  - [x] 实现嘴巴口型动画（CSS lip-sync动画）
  - [x] 创建AIDigitalHumanPage完整对话页面
  - [x] 修改HomePage添加AI数字人入口卡片
  - [x] 更新routes.tsx添加/ai-digital-human路由
  - [x] 代码质量检查通过（107个文件）
